...
class ApproveReportView(APIView):
    def post(self, request, Report_id, format=None):
        try:
            Report = Report_cat_three.objects.get(id=Report_id)
...
class SolvedTimelineView(ListAPIView):
    queryset = Report_cat_three.objects.filter(status="solved").order_by('-created_at')
    serializer_class = ReportSerializer


class ReportedTimelineView(ListAPIView):
    queryset = Report_cat_three.objects.filter(status="reported").order_by('-created_at')
    serializer_class = ReportSerializer


class ApprovedTimelineView(ListAPIView):
    queryset = Report_cat_three.objects.filter(status="approved").order_by('-created_at')
    serializer_class = ReportSerializer